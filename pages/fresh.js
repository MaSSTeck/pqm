...
import { GET_FRESH_MQUESTIONS } from '../src/graph/queries/getFreshMQuestionQuery';
...
export default withData(props => (
    ...
    <Container text style={{ marginTop: "2em" }}>
      <Header as="h1">Fresh Questions</Header>
      <p>
        ...
        <Grid.Row columns={3} >
          ...
          <Grid.Column mobile={16} tablet={16} computer={10}>
            <Container text>
                ...
                <Query query={GET_FRESH_MQUESTIONS}>
                    {({ loading, error, data }) => {
                      if (loading) return <div>Fetching...</div>
                      if (error) return <div>{error.message}</div>
                      if (data) return <div>{loadQuestion(data.getFreshMQuestion)}</div>
                    }}
                </Query>  
            </Container>
          </Grid.Column>
